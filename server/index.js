...
router.get('(/)', async (ctx) => {
  ...
  // Check if this is initial install (has 'host' parameter)
  const isInitialInstall = !!host;
  
  if (isInitialInstall) {
    // For initial install, check subscription status first
    try {
      // Create temporary session for subscription check
      const sessionId = `${shop}_offline`;
      ...
      if (session && session.accessToken) {
        ...
        if (!hasActiveSubscription) {
          ...
          const { confirmationUrl } = billingResponse.body.data.appSubscriptionCreate;
